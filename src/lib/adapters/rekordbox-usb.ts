...
export class RekordboxUsbAdapter implements DirectoryAdapter {
  ...
  async parseDirectory(handle: FileSystemDirectoryHandle): Promise<DirectoryImportResult> {
    ...
    if (!xmlContent) {
      throw new Error(
        'Rekordbox export nicht gefunden. Wähle das Wurzelverzeichnis deines USB-Sticks (muss einen PIONEER-Ordner enthalten).'
      )
    }
  ...
  private async findXml(handle: FileSystemDirectoryHandle): Promise<string | null> {
    try {
      const pioneer = await handle.getDirectoryHandle('PIONEER')
      const rekordbox = await pioneer.getDirectoryHandle('rekordbox')
      const file = await (await rekordbox.getFileHandle('export.xml')).getFile()
