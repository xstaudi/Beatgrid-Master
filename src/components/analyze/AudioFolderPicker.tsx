...
interface AudioFolderPickerProps {
  isLoading: boolean
...
export function AudioFolderPicker({ isLoading, trackCount, error, onSelect }: AudioFolderPickerProps) {
  ...
  return (
    ...
    >
      <CardContent className="flex flex-col items-center gap-4 py-8">
        {isLoading ? (
          <>
            <Folder className="h-10 w-10 text-primary animate-pulse" />
            <div className="w-full max-w-xs space-y-2 text-center">
              <p className="text-sm font-medium">Scanne Audio-Dateien...</p>
          ...
          <>
            ...
            <div className="text-center space-y-1">
              <p className="font-medium">Wähle deinen Musik-Ordner</p>
              <p className="text-sm text-muted-foreground">
                MP3, WAV, FLAC, AIFF – alle Unterordner werden gescannt
              </p>
            </div>
            {useFallback ? (
